...
Menu* getMainMenu(PanoSettings& settings){
    ...
    new Menu("Main Menu", 5, new BaseMenu* const[5] {
        ...
        new Menu("Pano", 6, new BaseMenu* const[6]{
            ...
            new RangeSelector("Manual Horiz FOV", &settings.horiz, 120, USE_EEPROM, NULL, 10, 360, 10),
            new RangeSelector("Manual Vert FOV", &settings.vert, 90, USE_EEPROM, NULL, 10, 180, 10)
        }),
       ...
       new Menu("Camera", 8, new BaseMenu* const[8] {
            ...
            new NamedListSelector("Processing Wait", &settings.post_wait, 100, USE_EEPROM, NULL, 7,
                (const char * const[]){"0.1s", "0.25s", "0.5s", "1s", "2s", "4s", "8s"},
                (const int[]){100, 250, 500, 1000, 2000, 4000, 8000}),
            new NamedListSelector("Shutter Mode", &settings.long_pulse, 0, USE_EEPROM, NULL, 2,
                (const char * const[]){"Normal", "Cont Bkt"},
                (const int[]){0, 1}),
            new RangeSelector("Shots #", &settings.shots, 1, USE_EEPROM, NULL, 1, 5, 1),
            new NamedListSelector("Aspect", &settings.aspect, 23, USE_EEPROM, NULL, 2,
                (const char * const[]){"P 2:3", "L 3:2"},
                (const int[]){23, 32}),
            new ActionItem("Test Shutter", onTestShutter)
        }),

        new NamedListSelector("Motors", &settings.motors_enable, 0, NO_EEPROM, NULL, 2,
            (const char * const[]){"On", "Off"},
            (const int[]){1, 0}),

        new NamedListSelector("Display", &settings.display_invert, 0, NO_EEPROM, NULL, 2,
